...
namespace NumberSystemConverter
{
    ...
    {
        M = 1000,
        CM = 900,   // Added in new RomanNumeralsType 'CM' for '900'
        D = 500,
        CD = 400,    // Added in new RomanNumeralsType 'CD' for '400'
        C = 100,
        XC = 90,    // Added in new RomanNumeralsType 'XC' for '90'
        L = 50,
        XL = 40,    // Added in new RomanNumeralsType 'XL' for '40'
        X = 10,
        IX = 9,     // Added in new RomanNumeralsType 'IX' for '9'
        V = 5,
        IV = 4,     // Added in new RomanNumeralsType 'IV' for '4'
        I = 1
    }
    #endregion
    public class RomanNumeralConverter
    {
        ...
        public string ConvertToRomanNumeral(int number)
        {
            ...
            foreach (var currentPair in romanNumeralList)
            {
                ...
                while (number >= currentPair.NumericValue)  // Changed from operator '>' to '>=' to make the loop only affect 
                {
        ...
        public bool CheckIfInputIsValid(string userInput)
        {
            int tryParse;
            bool tryTest = int.TryParse(userInput, out tryParse);

            if (tryTest == true)
            {
                if (tryParse <= 0 || tryParse > 3001)
                {
                    return false;
                }
                else
